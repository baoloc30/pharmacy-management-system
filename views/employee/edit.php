...
<?php if(isset($success)): ?><div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:12px 16px;margin-bottom:14px;color:#15803d;font-size:13px;"><i class="fas fa-check-circle"></i> <?php echo $success; ?></div><?php endif; ?>
<?php if(isset($error)): ?><div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:12px 16px;margin-bottom:14px;color:#dc2626;font-size:13px;"><i class="fas fa-exclamation-circle"></i> <?php echo $error; ?></div><?php endif; ?>
<div style="background:#fff;border-radius:14px;box-shadow:0 2px 14px rgba(0,0,0,.07);overflow:hidden;">
  ...
  <div style="padding:22px;">
    <form method="POST" action="" id="editEmpForm">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
        <div>
          ...
          <input type="text" name="hoTen" id="hoTen" value="<?php echo htmlspecialchars($employee['hoTen']); ?>" style="width:100%;padding:9px 12px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;">
        ...
        <div>
          ...
          <input type="text" name="soDienThoai" id="soDienThoai" value="<?php echo htmlspecialchars($employee['soDienThoai']??''); ?>" style="width:100%;padding:9px 12px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;">
          <span style="font-size:11px;color:#dc2626;" id="sdtErr"></span>
        </div>
        <div>
          <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;">Email</label>
          <input type="email" name="email" id="email" value="<?php echo htmlspecialchars($employee['email']??''); ?>" style="width:100%;padding:9px 12px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;">
          <span style="font-size:11px;color:#dc2626;" id="emailErr"></span>
        </div>
        <div>
          <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;">Ngày sinh</label>
          <input type="date" name="ngaySinh" value="<?php echo $employee['ngaySinh']??''; ?>" style="width:100%;padding:9px 12px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;">
        </div>
        <div>
        ...
        <div style="grid-column:1/-1;">
          ...
          <input type="text" name="diaChi" value="<?php echo htmlspecialchars($employee['diaChi']??''); ?>" style="width:100%;padding:9px 12px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;">
        </div>
      </div>
...
document.getElementById('editEmpForm').addEventListener('submit',function(e){
    let ok=true;
    ['hoTenErr','sdtErr','emailErr'].forEach(function(id){document.getElementById(id).textContent='';});
    ...
    if(sdt&&!/^[0-9]{10}$/.test(sdt)){document.getElementById('sdtErr').textContent='SĐT không hợp lệ';ok=false;}
